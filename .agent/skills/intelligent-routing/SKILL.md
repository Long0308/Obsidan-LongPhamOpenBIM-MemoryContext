...
### Platform-Aware Routing (Extension)

> **When a request matches a platform context**, use the platform coordinator instead of generic domain routing.

| Platform | Keywords | Coordinator Agent(s) | Escalation |
|----------|----------|---------------------|------------|
| **InsForge** | insforge, edge function, db query, storage bucket, realtime | `backend-specialist` + `database-architect` | → `security-auditor` if auth/RLS |
| **OpenSpec** | openspec, /opsx, propose, explore change, spec, apply change | `project-planner` | → `orchestrator` at apply phase |
| **Superpowers** | brainstorm, plan, execute plan, TDD, verify, ship | `project-planner` (plan) / `orchestrator` (execute) | Phase-dependent |
| **Obsidian** | obsidian, vault, note, canvas, .base, wikilink | `explorer-agent` | → `documentation-writer` for content |
| **UI Design** | stitch, design UI, component, layout, dark mode, theme | `frontend-specialist` | → `mobile-developer` if mobile |
| **Antigravity** | agent, skill, workflow, ag-kit, .agent/ | `orchestrator` | → `project-planner` for new skills |

### Platform Detection Priority

```javascript
// Platform detection runs BEFORE generic domain routing
function routeRequest(userMessage) {
    // 1. Check platform context first
    const platform = detectPlatform(userMessage);
    if (platform) {
        return getPlatformCoordinator(platform);
    }

    // 2. Fall back to generic domain routing
    const domain = detectDomain(userMessage);
    return getDomainAgent(domain);
}
```

### Cross-Platform Escalation

When a request spans multiple platforms, escalate to `orchestrator`:

```text
Example: "Deploy InsForge edge function with OpenSpec specs"
→ Detected: InsForge + OpenSpec
→ Escalate: orchestrator
→ Orchestrator coordinates: project-planner (specs) → backend-specialist (deploy)
```
