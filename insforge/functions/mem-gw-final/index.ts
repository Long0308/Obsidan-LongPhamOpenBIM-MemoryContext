...
// ── Save Session ──
if(req.method==='POST'&&p==='save-session'){const b=await req.json();const{data:m,error:e}=await db.from('memories').insert([{type:'long-term',category:'session',content:(b.content||'').substring(0,5000),tags:['session',b.ide,b.project].filter(Boolean),importance:0.9,source:'auto-memory'}]).select();if(e)return j({error:e.message},500);if(b.decisions?.length)await db.from('memories').insert(b.decisions.map((d:string)=>({type:'long-term',category:'decisions',content:d,tags:['decision',b.project].filter(Boolean),importance:0.85,source:'auto-memory'})));
  const dateStr=new Date().toISOString().split('T')[0];
  const ghPath=`LPOpenBIMAI/Agent-Memory/4-Sessions/session-${dateStr}-${b.ide||'agent'}-${b.project||'LPOpenBIMAI'}.md`;
  const g=b.push_github&&Deno.env.get('GITHUB_PAT')?await gp(ghPath,b.content||''):null;
  await db.from('sync_log').insert([{operation:'save',table_name:'memories',adapter_source:b.ide||'auto',adapter_target:b.push_github?'gh+if':'if',status:'success'}]);return j({status:'saved',id:m?.[0]?.id,gh:g,path:ghPath})}
