...
// In-memory context state (reset on cold start, updated by agent POSTs)
const ctxState: Record<string, {
  pct: number; est_tokens: number; model_limit: number;
  session_start: string; last_checkpoint: string;
  conversation_id: string; model: string; ide: string;
...
// ── Health ──
if(req.method==='GET'&&(!p||p==='health')){const[m,e,s]=await Promise.all([db.from('memories').select('id',{count:'exact',head:true}),db.from('entities').select('id',{count:'exact',head:true}),db.from('sync_log').select('id',{count:'exact',head:true})]);return j({status:'ok',v:'2.3',counts:{memories:m.count||0,entities:e.count||0,sync:s.count||0}})}

// ── Init Session ──
if(req.method==='POST'&&p==='init-session'){const b=await req.json();const proj=b.project||'LPOpenBIMAI';
  // Reset context state for this project
  ctxState[proj]={pct:2,est_tokens:2000,model_limit:200000,session_start:new Date().toISOString(),last_checkpoint:'None',conversation_id:b.conversation_id||'',model:b.model||'',ide:b.ide||'',breakdown:{chat:0,files:0,system:2000,tools:0}};
...
// ── Context Update (agent POSTs real context data here) ──
if(req.method==='POST'&&p==='context-update'){const b=await req.json();const proj=b.project||'LPOpenBIMAI';
  ctxState[proj]={
    ...
    session_start:ctxState[proj]?.session_start||new Date().toISOString(),
    last_checkpoint:b.checkpoint||new Date().toISOString(),
    conversation_id:b.conversation_id||ctxState[proj]?.conversation_id||'',
    model:b.model||ctxState[proj]?.model||'',
    ide:b.ide||ctxState[proj]?.ide||'',
    breakdown:b.breakdown||ctxState[proj]?.breakdown||{}
  };
  return j({status:'ok',pct:ctxState[proj].pct,est_tokens:ctxState[proj].est_tokens})}

// ── Context Status (dashboard polls this) ──
if(req.method==='GET'&&p==='context-status'){const proj=u.searchParams.get('project')||'LPOpenBIMAI';
  const state=ctxState[proj];
...
if(req.method==='POST'&&p==='sync-github'){if(!Deno.env.get('GITHUB_PAT'))return j({error:'No PAT'},500);
  // Sync last 5 sessions + current session note to GitHub
  ...
  // Also sync context state as a snapshot
  const ctxSnap=JSON.stringify(ctxState,null,2);
